...
function defaultValue(props:DeclarationReflection) {
   const value = props.comment?.blockTags.filter((tag) => tag.tag === "@default")
    if(props.defaultValue){
        return  props.defaultValue
    }
    return value?.flatMap((value) => value.content.map(item=>item.text)).join("")
}
        ...
        return (
            <tr>
                <td>
                    {
                        props.isDeprecated() ?
                            <del class="tsd-kind-property">⚠️{wbr(props.escapedName || props.name)}</del>
                           :
                            <span
                                class={classNames({
